...
emlo_dupes_to_remove = c(emlo_dupes$X1, emlo_dupes$X2,emlo_dupes$X3, emlo_dupes$X4) %>% 
  ...
  filter(!is.na(value)) %>% distinct(value) %>% filter(! value %in% to_keep) %>% mutate(reason = 'from_emlo_matches')
...
possible_dupes = work %>% 
  ...
  filter(cat1 != cat2) %>%
  arrange(standard_gregorian_date) %>% ungroup() %>% 
  distinct(emlo_letter_id_number)%>% mutate(reason = 'auto_duplicate')
...
possible_dupes_distinct_to_remove = work %>% 
  ...
  filter(n<4) %>% 
  group_by(standard_gregorian_date, author_emlo_id, recipient_emlo_id) %>% 
  summarise(all_cats = paste0(original_catalogue_name, collapse = ';'), emlo_letter_id_number) %>% separate(all_cats, into = c('cat1', 'cat2', 'cat3', 'cat4'), sep = ';') %>% 
  filter(cat1 != cat2) %>%
...
```{r}
rbind(emlo_dupes_to_remove, possible_dupes %>% rename(value = emlo_letter_id_number), test_cat_letter_ids%>% rename(value = emlo_letter_id_number),unknown_letters%>% rename(value = emlo_letter_id_number),letters_marked_to_delete%>% rename(value = emlo_work_letter_id))%>% distinct(value, reason) %>% write_csv('to_remove_list_with_unknown.csv')
```
...
new_to_remove = rbind(emlo_dupes_to_remove, possible_dupes %>% rename(value = emlo_letter_id_number), test_cat_letter_ids%>% rename(value = emlo_letter_id_number),letters_marked_to_delete%>% rename(value = emlo_work_letter_id))%>% distinct(value, .keep_all = T)
