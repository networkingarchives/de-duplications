...
emlo_dupes_to_remove = c(emlo_dupes$X1, emlo_dupes$X2,emlo_dupes$X3, emlo_dupes$X4) %>% 
  ...
  filter(! value %in% to_keep) %>% mutate(reason = 'from_emlo_matches')
...
possible_dupes = work %>% filter(!emlo_letter_id_number %in%emlo_dupes_to_remove$value ) %>% filter(!emlo_letter_id_number %in% letters_marked_to_delete$emlo_work_letter_id) %>% filter(!emlo_letter_id_number %in% test_cat_letter_ids$emlo_letter_id_number) %>% 
