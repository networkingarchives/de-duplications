...
```{r}
c(emlo_dupes_to_remove, possible_dupes, test_cat_letter_ids,unknown_letters) %>% as_tibble() %>% distinct(value) %>% write_csv('to_remove_list_with_unknown.csv')
```
...
new_to_remove = as.character(c(emlo_dupes_to_remove,  possible_dupes_distinct_to_remove, test_cat_letter_ids))
